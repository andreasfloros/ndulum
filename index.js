...
function updatePendulumMass() {
  ...
  if (pendulum.n < newN) {
    pendulum.thetas = pendulum.thetas.concat(
      Array(newN - pendulum.n).fill(pendulum.thetas[pendulum.thetas.length - 1])
    );
    pendulum.thetaDots = pendulum.thetaDots.concat(
      Array(newN - pendulum.n).fill(10 * Math.random() - 5)
    );
